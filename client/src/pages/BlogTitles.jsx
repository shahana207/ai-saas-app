...
const BlogTitles = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const prompt = `Generate 5 catchy blog titles for the keyword "${input}" in the category "${selectedCategory}". Return them in markdown list format.`;
    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      ...
      <form onSubmit={onSubmitHandler} className="w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200">
        ...
        <button
          ...
          from-[#C341F6] to-[#8E37EB] text-white px-4 py-2 mt-6 text-sm rounded-lg disabled:opacity-50"
        >
          {loading ? (
